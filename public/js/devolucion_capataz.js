document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!tablaBody) {
    console.warn('tablaDevolucion no encontrada en el DOM. Asegúrate que el HTML contiene <table id="tablaDevolucion"><tbody>...</tbody></table>');
    return;
  }
  if (!btnDevolver) {
  ...
  async function cargarPrestadas() {
    try {
      // UI: indicar carga mientras se solicita
  ...
  // Conectar SSE para actualizar la lista automáticamente cuando se creen préstamos o cambie el estado de herramientas
  try {
