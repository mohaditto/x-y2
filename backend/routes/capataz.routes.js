...
router.get("/historial/:capataz_id", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      ...
      SELECT 
        p.id AS prestamo_id,
        p.fecha_entrada AS fecha_prestamo,
        p.fecha_salida AS fecha_devolucion,
        p.estado,
...
router.get("/prestadas", async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      JOIN prestamo_items pi ON pi.prestamo_id = p.id
      JOIN herramientas h ON h.id = pi.herramienta_id
      WHERE p.estado = 'ACTIVO' AND pi.hora_salida IS NULL
    `);
...
router.get("/herramientas/dañadas", async (req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT 
        id,
    ...
    `);
    console.log(`/api/capataz/herramientas/dañadas -> ${rows.length} rows`);
    res.json(rows);
  } catch (error) {
